  ...
  return (
    <div key={match.id} className={`w-full card overflow-visible rounded-none !rounded-md border-4 border-gray-700 bg-card relative ${hasStreamInMatch() ? '!border-indigo-500' : ''}`}>
      ...
      <div className="p-4 px-8 pb-8 card-body">
        <span onClick={expand} className="active:scale-95 z-10 absolute right-0 left-0 bottom-4 hover:bg-indigo-500 hover:text-white hover:scale-110 transition mx-auto material-symbols-outlined cursor-pointer h-8 w-8 grid place-items-center rounded-full border-2 text-indigo-500 border-indigo-500">{expanded ? 'keyboard_arrow_up' : 'keyboard_arrow_down'}</span>
